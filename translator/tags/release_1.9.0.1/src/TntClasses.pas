...
uses
  Classes, SysUtils, Windows, {$IFNDEF COMPILER_6_UP} Controls, {$ENDIF} ActiveX;
...
uses
  {$IFDEF COMPILER_6_UP} RTLConsts, {$ELSE} Consts, Forms, {$ENDIF} ComObj,
...
var
  P, Start, _End: PWideChar;
  S: WideString;
begin
  // (p3)
  // The original method would break on the first #0 in the input string.
  // This method chugs along until the real end of the string (which is the correct behavior)
  BeginUpdate;
  try
    Clear;
    P := Pointer(Value);
